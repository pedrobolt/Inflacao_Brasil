...
bd <- bd %>% 
  mutate(Presidente = case_when(
    ...
    date > "2018-12-01" & date <= "2022-12-01" ~ "Jair Bolsonaro",
    TRUE ~ "Lula 3"
  ))
...
ggplot() +
  ...
  annotate(geom = "text", x = as.Date("2021-01-01"), y = 3.5,
          label = "Jair\nBolsonaro", size = 4) +
  # Lula 3
  geom_vline(xintercept=as.Date("2023-01-01"), col = "blue", linetype = 2, size = 0.65) +
  annotate(geom = "text", x = as.Date("2024-06-01"), y = 3.5,
           label = "Lula 3", size = 4) +
    labs(title = "Comportamento da Inflação brasileira",
